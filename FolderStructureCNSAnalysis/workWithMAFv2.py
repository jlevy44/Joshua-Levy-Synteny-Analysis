        ...
        if skipSegment == 0: # if staying with segment for all species, write the lines to output bed and add respective
            ...
            if all(countSeq[species] < 2 for species in countSeq.keys()) and sum([countSeq[key] for key in countSeq.keys()]) > 1:
                ...
                if mafAnalysisStructure[conditional] == None or mafAnalysisStructure[conditional] == []: #FIXME
                    mafAnalysisStructure[conditional] = {'CS': defaultdict(list),'ProteinCoding_CS': defaultdict(list),'Mixed_CS': defaultdict(list),'NonProteinCoding_CS': defaultdict(list)}
...
for conditional in mafAnalysisStructure.keys():
    ...
    for MASeg in mafAnalysisStructure[conditional]['CS']:
        ...
        for sequence in mafAnalysisStructure[conditional]['CS'][MASeg].split('>'):
            if sequence:
    ...
    for analysisType in ['CS','ProteinCoding_CS','Mixed_CS','NonProteinCoding_CS']:
        ...
        outputFinalFastaFile.write(''.join('>%s\n%s\n'%(species,conservedSeqs[analysisType][species]) for species in conservedSeqs[analysisType].keys()))
