...
for sample in listSamples:
    ...
    bbCommands = [headSh.replace('module load samtools/1.3.1\n','module unload samtools\nmodule load samtools/1.4\n')] + ['rm -r ref\nrm BBmapped.bed'] + ['bbmap.sh fastareadlen=300 threads=6 in=%s.fa ref=%s minid=0.97 ef=0.01 outm=BBmapped.bam ambiguous=toss'%(sample,root+'referenceGenomes/%s/'%CDSspecies+fastaNucOld),
